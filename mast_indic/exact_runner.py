"""CLI: run the exact/sparse-only agent (`exact_agent.py`) over a language's
queries, write submission JSONL.

Output: runs/{chat_model}/exact_{language}.jsonl -- same record shape as
`runner.py`/`interact_runner.py`, so it drops straight into
`mast_indic/eval.py` unchanged. `retriever` is tagged `exact-search` (no
embedding model -- this agent's retrieval never calls one).
"""
...
from .exact_agent import ExactSearchAgent
...
def run_language(language: str, limit: int | None = None, save_transcripts: bool = False) -> str:
    ...
    agent = ExactSearchAgent(search_index=search_index)

    out_dir = os.path.join(config.runs_dir, config.chat_model)
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"exact_{language}.jsonl")
